...
#if DEBU
                print(String(format: "OUT (0x%2X), A", port))
#endif
                wait(11)
                return
            default:
...
*/
    private mutating func add(_ b: UInt8) {
        let a = registers[A]
        let sum = a + b
        registers[A] = sum
        var f = (registers[F] & 0x28)
        if (sum & 0x80) > 0 {
            f |= Flags.S.rawValue
        }
        if sum == 0 {
        ...
        if (a >= 0x80 && b >= 0x80 && Int8(sum) > 0) || (a < 0x80 && b < 0x80 && Int8(sum) < 0) {
            f |= Flags.PV.rawValue
        }
        if sum > 0xFF {
            f |= Flags.C.rawValue
    ...
    private mutating func adc(_ b: UInt8) {
        let a = registers[A]
    ...
    private mutating func sub(_ b: UInt8) {
        let a = registers[A]
        let diff = a - b
        registers[A] = diff
        var f = registers[F] & 0x28
        ...
        if (a >= 0x80 && b >= 0x80 && Int8(diff) > 0) || (a < 0x80 && b < 0x80 && Int8(diff) < 0) {
            f |= Flags.PV.rawValue
        }
        f |= Flags.N.rawValue
        if Int8(diff) < 0 {
    ...
    private mutating func sbc(_ b: UInt8) {
        let a = registers[A]
        let c = registers[F] & 0x01
        let diff = a - b - c
        registers[A] = diff
        var f = registers[F] & 0x28
        if (diff & 0x80) > 0 {
    ...
    private mutating func and(_ b: UInt8) {
        let a = registers[A]
        let res = a & b
        registers[A] = res
        var f = registers[F] & 0x28
        ...
        f |= Flags.H.rawValue
        if Z80.parity(UInt16(res)) {
            f |= Flags.PV.rawValue
        }
        registers[F] = f
    }

    private mutating func or(_ b: UInt8) {
        let a = registers[A]
        let res = a | b
        registers[A] = res
        var f = registers[F] & 0x28
    ...
    private mutating func xor(_ b: UInt8) {
        let a = registers[A]
        let res = a ^ b
        registers[A] = res
        var f = registers[F] & 0x28
    ...
    private mutating func cmp(_ b: UInt8) {
        let a = registers[A]
        let diff = a - b
        var f = registers[F] & 0x28
        if (diff & 0x80) > 0 {
    ...
    private mutating func inc(_ b: UInt8) -> UInt8 {
        let sum = b + 1
        var f = registers[F] & 0x28
        if (sum & 0x80) > 0 {
            f |= 0x80
    ...
    private mutating func dec(_ b: UInt8) -> UInt8 {
        let sum = b - 1
        var f = registers[F] & 0x28
        if (sum & 0x80) > 0 {
            f = f | 0x80
    ...
    private mutating func swapReg8(_ register1: Int, _ register2: Int) {
        let t = registers[register1]
        registers[register1] = registers[register2]
        registers[register2] = t
    }
...
#if DEBUG
    private static func jName(_ condition: Int) -> String {
        switch condition {
            case 0:
                return "NZ"
            case 1:
